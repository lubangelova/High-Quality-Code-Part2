...
namespace Exceptions_Homework.Tests
{
    public class UtilitiesTester
    {
        public static void RunSubSubsequeneTest()
        {
            try
            {
                var substr = ExceptionsUtilities.Subsequence("Hello!".ToCharArray(), 2, 7);
            ...
            foreach (var item in testData)
            {
                try
                {
                    var subarr = ExceptionsUtilities.Subsequence(item.Key, item.Value[0], item.Value[1]);
        ...
        public static void RunExtractEndingTest()
        {
            ...
            foreach (var data in testData)
            {
                try
                {
                    Console.WriteLine(ExceptionsUtilities.ExtractEnding(data.Key, data.Value));
                }
                catch (ArgumentNullException ex)
                {
                    Console.WriteLine(ex.Message);
        ...
        public static void RunCheckPrimeTest()
        {
            ...
            for (int i = 0; i < numbers.Length; i++)
            {
                try
                {
                    ExceptionsUtilities.CheckPrime(numbers[i]);
